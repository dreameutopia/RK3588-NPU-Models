                                                ...
                                                float scale_w, float scale_h, ppocr_det_result* results)
{
    ...
    for (int _i = 0; _i < num_contours; _i++) {
        ...
        if (db_box_type == "poly"){
            ...
            for(size_t _k = 0; _k < points.size(); _k++) {
    ...
    for (size_t n = 0; n < boxes.size(); n++) {
        ...
        for (size_t m = 0; m < boxes[0].size(); m++) {
            boxes[n][m][0] = int(std::min(std::max(boxes[n][m][0], 0), det_out_w - 1));
            boxes[n][m][1] = int(std::min(std::max(boxes[n][m][1], 0), det_out_h - 1));
        }
    ...
    results->count = 0;
    for (size_t n = 0; n < root_points.size(); n++) {
